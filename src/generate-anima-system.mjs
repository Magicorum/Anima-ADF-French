...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'anima', 'Anima', and 'ANIMA'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.anima/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.anima/g),
   ...
   * Rename files.
   *
   * Rename files that had anima in their name, such as
   * css/anima.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*anima*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('anima', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-anima-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob Anima's files so that we can process them.
    glob('*', {ignore: ['node_modules/**'] }).then(files => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace anima name mentions in files.
      generator.replaceFileContents();
      // Rename files that had anima in their name.
